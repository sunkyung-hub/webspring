...
<header>
	...
	<!-- ${sub} 숫자나옴. length: 단어의 개수 파악 -->
게시판 제목: ${fn:substring(subject, 0, 10)} ${jum}<br><!-- ex. 게시판 제목: 공지사항 3월 이벤 ../10단어까지만 출력-->

	<!-- 해당 페이지 jstl 변수를 생성하여 값을 top.jsp로 전달되지 않음 -->
	<cr:set var="a" value="연습"/>
	<!--import: 외부 페이지 로드
		url(속성): 외부페이지 경로 및 파일명 
		param(import 속성 태그 안에만 적용): 파라미터 이름으로 해당 페이지에 전달하는 방식(name)-->
...
<!-- fn에 jsp변수값을 직접 적용하지 못함. 아래와 같이 한번 거쳐야 함 -->
